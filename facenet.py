...
from compare.simpling import Simpling
import torch.optim as optim
import traceback
from match.simpling import FaceMatchDataset
...
if __name__ == '__main__':
    ...
    cls_num = FaceMatchDataset.cls_num()
    # cls_num =10000
    print(cls_num)
    # cls_num = 10000
    net = Net(cls_num)
    ...
    state_dict.update(torch.load("../param/fnet_model_v18_full.pt"))
    net.load_state_dict(state_dict)
    center_loss_fn.load_state_dict(torch.load("../param/fnet_center_v18.pt"))

    optimizer0 = optim.Adam(net.parameters())
    optimizer1 = optim.Adam(center_loss_fn.parameters())

    dataset = FaceMatchDataset()
    dataloader = DataLoader(dataset, batch_size=200, shuffle=True, num_workers=2, drop_last=True)
    ...
    while True:
        try:
            ...
            for i, (_img_data, _cls) in enumerate(dataloader):
                ...
                print(k, i, "............................................................................... loss:",
                      loss.cpu().data.numpy(), " cls_loss:", cls_loss.cpu().data.numpy(), " center_loss:",
                      center_loss.cpu().data.numpy())

                k += 1
                if k % 100 == 0:
                    # 分类层权重
                    torch.save(net.state_dict(), "../param/fnet_model_v18_full.pt")
                    dic = net.state_dict()
                    dic.pop('fc2.weight')
                    dic.pop('fc2.bias')

                    torch.save(dic, "../param/fnet_model_v18.pt")
                    torch.save(center_loss_fn.state_dict(), "../param/fnet_center_v18.pt")
                    print("save ........")
                    # exit()
        except Exception:
            traceback.print_exc()
